...
Named for **Abraham Van Helsing**, the scholar, physician and adversary of Dracula, conceived as the spiritual daylight counter that [Dracula Theme](https://draculatheme.com/) always needed: less crypt, more study.
...
- Fixture expectations: `generator/config/vscode-fixtures.yml`
...
- Generated role matrix: `docs/helsing-vscode-role-matrix.md`

Open this directory as the VS Code workspace and press `F5` to launch the **Run Helsing Theme** Extension Development Host configuration. The fixtures and token-inspection procedure are documented in `examples/README.md`.

To install dependencies, validate the generated contract, and package from this directory:
...
If you intentionally change the palette or VS Code mapping, run `npm run theme:generate` before `npm run theme:check`. Packaging invokes the same validation automatically and refuses stale generated files.
